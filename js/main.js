...
const customFunction = () => {
  const dateOptions = [];
  for (let i = 1; i <= 31; i++) {
    dateOptions.push(`<option data-tokens="${i}">${i}</option>`);
  }
  $("#slidercontentInputTravelDate").append(dateOptions.join(""));
};

const booknowOnsubmit = (e) => {
  e.preventDefault();
  const contactBy = e.target["contactby"].value;
  if (
    contactBy === "Contact By" ||
    e.target["date"].value === "Date" ||
    e.target["month"].value === "Month" ||
    e.target["categories"].value === "Categories"
  ) {
    return;
  }
  ...
  } else if (contactBy === "Whatsapp") {
    window.open(
      `https://wa.me/6283892568969?text=${encodeURIComponent(message)}`
    );
  } else if (contactBy === "Telegram") {
    window.open(`https://t.me/ganbalitour?text=${encodeURIComponent(message)}`);
  }
};
